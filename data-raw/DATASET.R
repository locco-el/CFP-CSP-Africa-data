...
rawdata[, `:=`(net_prof = CSPandCFP::coerce_num_cln(net_prof),
               tot_asst = CSPandCFP::coerce_num_cln(tot_asst),
               bk_eqt = CSPandCFP::coerce_num_cln(bk_eqt),
               market_value = CSPandCFP::coerce_num_cln(market_value))]


# calc CFP variables
rawdata[, `:=`(return_on_assets = net_prof/tot_asst,
                return_on_equity = net_prof/bk_eqt,
                tobins_q = market_value/tot_asst)
        ][bk_eqt == 0, return_on_equity := NA
          ][tot_asst == 0, `:=`(return_on_assets = NA,
                                tobins_q = NA)
...
rawdata[return_on_equity > 50 | return_on_equity < -50, return_on_equity := NA
        ][tobins_q > 50 | tobins_q < -50, tobins_q := NA
          ][return_on_assets > 50 | return_on_assets < -50, return_on_assets := NA]

# calc firm age vars
rawdata[, `:=`(listing_age = reporting_year - listing_year,
...
rawdata[, `:=`(Country = factor(listing_country),
...
library(data.table)
rawdata[, lag_return_on_assets := shift(return_on_assets, 1L, 'lag'), by = 'company'
        ][, lag_return_on_equity := shift(return_on_equity, 1L, 'lag'), by = 'company'
          ][, lag_tobins_q := shift(tobins_q, 1L, 'lag'), by = 'company']



reg_vars <- c('Country', 'Multinational', 'Sector',
...
csr_vars <- names(rawdata)[grepl('com_|ene_|prod_|emi_|hr_|env_|oth_',
                                  names(rawdata))]
...
hr = list(
  ...
  n_hr_benefits = c('6', '12', '13', '21', '22', '23', '24',
                    '26', '27', '28', '29'),
...
csr_varlist <-
  unlist(lapply(csr_stems,
                function(x) lapply(get(x),
                                   function(y)
...
pca_data <- (cbind(reg_data[, .(Sector, Country)],
